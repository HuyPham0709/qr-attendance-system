...
import { AuditLogFormModal } from '../components/audit/AuditLogFormModal'
import { AuthUser } from '../services/authService'
import { AuditLogItem, deleteAuditLog, downloadAuditReport, listAuditLogs } from '../services/auditService'
...
export function StaffAuditScreen({ user }: StaffAuditScreenProps) {
  ...
  const [editingLog, setEditingLog] = useState<AuditLogItem | null>(null)
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      if (auditResponse) { setAuditLogs(auditResponse.data); setAuditPages(Math.max(1, auditResponse.pagination.pages)); setAuditTotal(auditResponse.pagination.total) }
  ...
  async function removeAuditLog(log: AuditLogItem) {
    if (!window.confirm('Xóa audit log này? Thao tác sẽ thay đổi audit trail.')) return
    try { await deleteAuditLog(log._id); showToast('Đã xóa audit log'); setAuditPage(1); setResult('') }
    catch (err: any) { showToast(err.message || 'Không thể xóa audit log', 'error') }
  }
  ...
  return <div className="p-6 space-y-5">
    <div className="flex items-center justify-between"><div><h1 className="text-xl font-bold text-slate-900">{superAdmin ? 'Security Audit' : 'Staff & Security Audit'}</h1><p className="text-sm text-slate-500">{superAdmin ? 'System-wide audit log' : 'Staff assignments · Audit log'}</p></div><div className="flex gap-2"><Button variant="primary" onClick={() => { setEditingLog(null); setShowLogForm(true) }}>+ Add Log</Button><Button variant="secondary" onClick={() => setShowImport(true)}>Import Report</Button><Button variant="secondary" onClick={() => setShowExport(true)}>Export Report</Button></div></div>
    ...
    <AuditLogFormModal open={showLogForm} events={events} log={editingLog} onClose={() => { setShowLogForm(false); setEditingLog(null) }} onSaved={message => { showToast(message); setAuditPage(1); setResult('') }} />
